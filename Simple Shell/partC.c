...
// To redirect some content into the txt file
void Redirect_Execution(char** adj,int Number_Of_Steps,int mode){
	int Program_Runner,fd;
	char *argv[100];
	Erase_White_Space(adj[1]);
	Token_maker(argv,&Program_Runner,adj[0]," ");
	if(fork()==0){

		fd=open(adj[1],O_WRONLY); 
...
void redirect_caller(){
	Token_maker(Reductant,&Number_Of_Steps,adj,">");
	if(Number_Of_Steps==2)Redirect_Execution(Reductant,Number_Of_Steps, 1);
	else printf("Output Redirection is Wrong");
}
...
void command_check(){
	...
	int if_pipe = 0, if_and_and = 0, if_redirect = 0;
	
	for(int i = 0; i < sizeof(adj); i++){
		if(adj[i] == '|') if_pipe = 1;
		if(adj[i] == '&') if_and_and = 1;
		if(adj[i] == '>' ) if_redirect = 1;
	}
	
	if_pipe == 1 ? pipe_caller() : if_and_and == 1 ? and_and_caller() : if_redirect == 1 ? redirect_caller() : basic_function_caller();
